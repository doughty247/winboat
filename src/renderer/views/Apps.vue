        ...
        <div v-if="winboat.isOnline.value" class="px-2">
                <TransitionGroup v-if="apps.length" name="apps" tag="x-card" class="grid gap-4 bg-transparent border-none app-grid">
                    ...
                    >
                        ...
                        <WBContextMenu>
                            <WBMenuItem v-if="wbConfig.config.desktopLauncherEnabled" @click="createDesktopLauncher(app)">
...
async function createDesktopLauncher(app: WinApp) {
    try {
        await launcherMgr.createLauncher(app);
    } catch (error) {
        console.error('Failed to create desktop launcher:', error);
    }
}
